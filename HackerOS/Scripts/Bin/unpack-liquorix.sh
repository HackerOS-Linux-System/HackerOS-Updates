...
########################################
# 1) Pobranie pliku z GitHub
########################################

echo "${LOGPREFIX} Pobieram plik z GitHub: ${GITHUB_RAW} -> ${TMPFILE}"
if ! command -v curl >/dev/null 2>&1; then
  echo "${LOGPREFIX} curl nie znalezione, instaluję curl..."
  sudo apt update
...
########################################
# 8) USUWANIE JĄDRA DEBIAN — NA SAMYM KOŃCU
########################################

echo "${LOGPREFIX} Uruchamiam remove-debian-kernel.sh"

if [ -x "/usr/share/HackerOS/Scripts/Bin/remove-debian-kernel.sh" ]; then
    sudo /usr/share/HackerOS/Scripts/Bin/remove-debian-kernel.sh
else
    echo "${LOGPREFIX} UWAGA: remove-debian-kernel.sh nie istnieje lub nie ma +x"
fi

########################################
# 9) Zapis końcowego statusu do ~/.config/hackeros/kernel.hacker
########################################

mkdir -p "${HOME}/.config/hackeros"
echo "[liquorix]" > "${HOME}/.config/hackeros/kernel.hacker"

echo "${LOGPREFIX} Zapisano ~/.config/hackeros/kernel.hacker"
